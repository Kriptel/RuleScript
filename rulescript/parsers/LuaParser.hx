...
class LuaParser extends Parser
{
	...
	**/
	...
	override public function parse(s:String):Expr
	{
		initParser('rulescript', 0);
		input = s;
		readPos = 0;
		var a = new Array<Expr>();
		a.push(mk(EVar('print', null, mk(EIdent('trace')))));
		while (true)
		{
			var tk = token();
			if (tk == TEof)
	...
	function parseExprNext(e1:Expr)
	{
		...
		switch (tk)
		{
			...
			case TPOpen:
				var e = parseExprNext(mk(ECall(e1, parseExprList(TPClose)), pmin(e1)));
				...
				return e;
			case TBrOpen:
				var e = parseExprNext(mk(ECall(e1, parseExprList(TBrClose)), pmin(e1)));
	...
	function parseFunctionDecl()
	{
		...
		var a = new Array<Expr>(), tk;
		while (true)
		{
			parseFullExpr(a);
			tk = token();
			if (tk.match(TId('end')) || (resumeErrors && tk == TEof))
				break;
			push(tk);
		}

		return {args: args, ret: ret, body: mk(EBlock(a))};
	}
						...
						while (true)
						{
							...
							if (!opPriority.exists(op) && opPriority.exists(pop))
							{
								if (op == "--")
								{
									...
									if (char == '['.code && StringTools.fastCodeAt(input, readPos + 1) == '['.code)
